...
import it.vgi.il.config.KafkaProperties;
...
class VgiIlOauthSecurityKafkaResourceIT {
    ...
    void setup() {
        ...
        VgiIlOauthSecurityKafkaResource kafkaResource = new VgiIlOauthSecurityKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/vgi-il-oauth-security-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/vgi-il-oauth-security-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
